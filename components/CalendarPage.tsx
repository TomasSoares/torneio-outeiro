        ...
        <div style={{ position: 'relative' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <Pill color={T.mute} bg={T.surf2} T={T}>
              {match.group ? `J${match.jornada}` : (match.round === 'SF1' ? 'Meia-Final 1' : match.round === 'SF2' ? 'Meia-Final 2' : match.round === 'F' ? 'Final' : '3.º/4.º')}
            </Pill>
          </div>
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
        <div className="mono" style={{ fontSize: 10, color: T.mute2, letterSpacing: 0.8 }}>
          {match.group ? match.time : `${match.time} · ${match.round === 'SF1' ? 'MF1' : match.round === 'SF2' ? 'MF2' : match.round === 'F' ? 'Final' : '3º/4º'}`}
        </div>
        {played ? (
